...
import { EnvelopeSvg } from '@/screens/feed/comment/_svg';
import { Post } from '@/screens/feed/_model/feed.model';
import { useUserStore } from '@/_state/useUserStore';
import { requestReviewLib } from '@/screens/feed/comment/_lib/requestReview.lib';

interface DetailRequestReviewActionProps {
  post: Post;
}

export function DetailRequestReviewAction({ post }: DetailRequestReviewActionProps) {
  ...
  const { mutate: handleRequestReview, isPending } = useMutation({
    ...
    onError: error => {
      console.error('Request review error in detail:', error);
